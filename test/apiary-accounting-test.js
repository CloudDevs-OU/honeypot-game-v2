...
describe("ApiaryAccounting", async function() {
    ...
    it("should successfully register apiary", async function() {
        ...
        expect(apiary.owner).eq(owner.address);
        expect(apiary.lastClaimTimestamp).not.eq(0);
        expect(apiary.lastDeferredPayoutTimestamp).eq(apiary.lastClaimTimestamp);
    })

    it("should failed to register apiary with message 'Apiary is already registered'", async function() {
        const [, owner] = await ethers.getSigners();
        await expect(accounting.registerApiary(owner.address)).revertedWith("Apiary is already registered");
    })

    it("should failed to register apiary with message 'Only admin'", async function() {
        const [,, thirdPerson] = await ethers.getSigners();
        await expect(accounting.connect(thirdPerson).registerApiary(thirdPerson.address)).revertedWith("Only admin");
    })
    ...
    it("should fail to update moodRecoveryTime with message 'Only admin'", async function() {
        const [,, thirdPerson] = await ethers.getSigners();
        await expect(accounting.connect(thirdPerson).setMoodRecoveryTime(0)).revertedWith("Only admin");
    })
    ...
    it("should successfully set item bonus percents", async function() {
        ...
        expect(percents[0]).eq(10);
        expect(percents[1]).eq(20);
        expect(percents[2]).eq(30);
    })
    ...
    it("should successfully set set bonus percents", async function() {
        ...
        expect(percents[0]).eq(100);
        expect(percents[1]).eq(200);
        expect(percents[2]).eq(300);
    })
